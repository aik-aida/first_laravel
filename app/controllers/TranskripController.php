			...
			if (strcmp(strtolower($data[0]->plaintext), strtolower("TRANSKRIP MATA KULIAH"))==0) {
			    ...
			    foreach( $data as $element){
			    	...
			    	if($no==3) {
			    			...
			    			$data_transkrip->nama = $id[1];
			    	}
			    	elseif ($no>($tahap1+5) && $tahap2==0 && $dont==false && $ipk==0 && $sks==0) {
			    			switch ($no%5) {
			    				case $tahap1_1:
			    					$data_transkrip->mkpersiapan[$persiapan]=(object) array("kode"=>$element);
			    					//$mk->kode = $element;
			    					break;
			    				case $tahap1_2:
			    					//array_push($data_transkrip->mkpersiapan[$persiapan], var)
			    					//$mk->nama = $element;
			    					$data_transkrip->mkpersiapan[$persiapan]->nama=$element;
			    					break;
			    				case $tahap1_3:
			    					//$mk->sks = $element;
			    					$data_transkrip->mkpersiapan[$persiapan]->sks=$element;
			    					break;
			    				case $tahap1_4:
			    					$data_transkrip->mkpersiapan[$persiapan]->catatan=$element;
			    					break;
			    				case $tahap1_5:
			    					//$mk->nilai = $element;
			    					$data_transkrip->mkpersiapan[$persiapan]->nilai=$element;
			    					$persiapan++;
			    	...
			    	elseif ($no>($tahap2+5) && $tahap2>$tahap1 && $dont==false && $ipk==0 && $sks==0) {
			    		switch ($no%5) {
			    				case $tahap2_1:
			    					$data_transkrip->mksarjana[$sarjana]=(object) array("kode"=>$element);
			    					break;
			    				case $tahap2_2:
			    					$data_transkrip->mksarjana[$sarjana]->nama=$element;
			    					break;
			    				case $tahap2_3:
			    					$data_transkrip->mksarjana[$sarjana]->sks=$element;
			    					break;
			    				case $tahap2_4:
			    					$data_transkrip->mksarjana[$sarjana]->catatan=$element;
			    					break;
			    				case $tahap2_5:
			    					$data_transkrip->mksarjana[$sarjana]->nilai=$element;
			    					$sarjana++;
			    					break;
			    				default:
			    					break;
